...
import com.hlcy.yun.sip.gb28181.bean.DeviceChannel;
import com.hlcy.yun.sip.gb28181.bean.RecordInfo;
import com.hlcy.yun.sip.gb28181.bean.RecordItem;
import com.hlcy.yun.sip.gb28181.message.handler.RequestHandler;
import com.hlcy.yun.sip.gb28181.notification.event.AlarmEvent;
...
import com.hlcy.yun.sip.gb28181.util.XmlUtil;
...
import java.util.*;
import static com.hlcy.yun.sip.gb28181.message.handler.request.MessageRequestHandler.CmdTypeStrategyFactory.*;
import static com.hlcy.yun.sip.gb28181.notification.PublisherFactory.getDeviceEventPublisher;
import static com.hlcy.yun.sip.gb28181.operation.callback.DeferredResultHolder.*;
import static com.hlcy.yun.sip.gb28181.util.XmlUtil.*;
...
public class MessageRequestHandler extends RequestHandler {
    ...
    private String getCmdTypeFrom(RequestEvent event) {
        return getTextOfChildTagFrom(getRootElementFrom(event), "CmdType");
    }
    ...
    class CatalogStrategy extends AbstractCmdTypeStrategy {
        ...
        protected void handleMessage(RequestEvent event) {
            ...
            getDeviceEventPublisher().publishEvent(new DeviceEvent(device));
        ...
        private void setChannelMapForDevice(Device device, Element deviceListElement) {
            ...
            while (deviceListIterator.hasNext()) {
                ...
                channelMap.put(channelDeviceId, new DeviceChannel()
                        .setName(getTextOf(itemDevice.element("Name")))
                        .setChannelId(channelDeviceId)
                        .setStatus(Integer.parseInt(getOrDefaultTextOf(itemDevice.element("Status"), "1")))
                        .setManufacture(getTextOfChildTagFrom(itemDevice, "Manufacturer"))
                        .setModel(getTextOfChildTagFrom(itemDevice, "Model"))
                        .setOwner(getTextOfChildTagFrom(itemDevice, "Owner"))
                        .setCivilCode(getTextOfChildTagFrom(itemDevice, "CivilCode"))
                        .setBlock(getTextOfChildTagFrom(itemDevice, "Block"))
                        .setAddress(getTextOfChildTagFrom(itemDevice, "Address"))
                        .setParental(Integer.parseInt(getOrDefaultTextOf(itemDevice.element("Parental"), "0")))
                        .setParentId(getTextOfChildTagFrom(itemDevice, "ParentId"))
                        .setSafetyWay(Integer.parseInt(getOrDefaultTextOf(itemDevice.element("SafetyWay"), "0")))
                        .setRegisterWay(Integer.parseInt(getOrDefaultTextOf(itemDevice.element("RegisterWay"), "1")))
                        .setCertNum(getTextOfChildTagFrom(itemDevice, "CertNum"))
                        .setCertifiable(Integer.parseInt(getOrDefaultTextOf(itemDevice.element("Certifiable"), "0")))
                        .setErrCode(Integer.parseInt(getOrDefaultTextOf(itemDevice.element("ErrCode"), "0")))
                        .setEndTime(getTextOfChildTagFrom(itemDevice, "EndTime"))
                        .setSecrecy(getTextOfChildTagFrom(itemDevice, "Secrecy"))
                        .setIpAddress(getTextOfChildTagFrom(itemDevice, "IPAddress"))
                        .setPort(Integer.parseInt(getOrDefaultTextOf(itemDevice.element("Port"), "0")))
                        .setPassword(getTextOfChildTagFrom(itemDevice, "Password"))
                        .setLongitude(Double.parseDouble(getOrDefaultTextOf(itemDevice.element("Longitude"), "0.00")))
                        .setLatitude(Double.parseDouble(getOrDefaultTextOf(itemDevice.element("Latitude"), "0.00"))));
            }
        }
    }
    ...
    static class DeviceInfoStrategy extends AbstractCmdTypeStrategy {
        ...
        protected void handleMessage(RequestEvent event) {
            ...
            Device device = new Device(deviceId)
                    .setName(getTextOfChildTagFrom(rootElement, "DeviceName"))
            ...
            // 向管理中心发布设备事件
            getDeviceEventPublisher().publishEvent(new DeviceEvent(device));
            DeferredResultHolder.setDeferredResultForRequest(CALLBACK_CMD_DEVICE_INFO + deviceId, device);
        }
    }
    ...
    static class AlarmStrategy extends AbstractCmdTypeStrategy {
        ...
        protected void handleMessage(RequestEvent event) {
            ...
            Device device = new Device(deviceId).setName(getTextOfChildTagFrom(rootElement, "DeviceName"))
                    .setManufacturer(getTextOfChildTagFrom(rootElement, "Manufacturer"))
                    .setModel(getTextOfChildTagFrom(rootElement, "Model"))
                    .setFirmware(getTextOfChildTagFrom(rootElement, "Firmware"));
            // 向管理中心发布设备告警事件
            getDeviceEventPublisher().publishEvent(new AlarmEvent(device));
        }
    }
    ...
    static class RecordInfoStrategy extends AbstractCmdTypeStrategy {
        ...
        protected void handleMessage(RequestEvent event) {
            ...
            if (isFullRecordList(recordInfo, sn)) {
                DeferredResultHolder.setDeferredResultForRequest(CALLBACK_CMD_RECORD_INFO + deviceId, recordInfo);
            }
        }
